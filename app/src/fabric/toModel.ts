...
function stageColumnsOf(
  ...
  // `ranks[i]` is in range by construction — same array, same index.
...
function stageLayerName(inColumn: Node[], refs: Record<string, SandboxTableRef>): string {
  for (const n of inColumn) {
    ...
    // rank >= 0 is only produced by stageRank finding it in STAGE_ORDER.
    const stage = rank >= 0 ? STAGE_ORDER[rank]! : null
...
export function sequenceToModel(
  ...
  const flowOf = new Map<
    string,
    // `| undefined` for exactOptionalPropertyTypes: transform is set from a
    // nullable field and is explicitly-undefined as often as it is absent.
    { from: string; column: string; transform?: string | undefined; via: string }
  >()
...
export function defaultModelName(steps: Step[]): string {
  ...
  // Guarded by the length check above, so the first element is always there.
